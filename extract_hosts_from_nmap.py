                ...
                for service in host.services:
                    if service.state == 'open':
                        ...
                        if port == 'web':
                            ...
                            if int(port) == int(service.port):
                                targets.append(host.address)
                            
        except NmapParserException:
            #print(f"File {nmap_xml_file} is not yet ready.")
            pass
...
def main():
    ...
    parser.add_argument('--port', help='Which port to lookup? Special port "www" matches any web related port.', metavar='PORT')
